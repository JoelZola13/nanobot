...
import { Loader2, Mic, SlidersHorizontal, Sparkles, Square, X } from 'lucide-react';
...
export function FloatingGenerateBox({
  ...
  const isVoiceCaptureInProgress = isRecording || isTranscribing;
  const captureStatusText = isTranscribing
    ? 'Transcribing your speech...'
    : isRecording
      ? 'Recording in progress. Click the square button to stop and generate.'
      : '';
  ...
  async function handleRecordAndGenerate() {
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)}>
            <div className="flex gap-2">
              ...
              {captureStatusText ? (
                <p className="text-[10px] text-muted-foreground mt-1">{captureStatusText}</p>
              ) : null}
              ...
              <div className="relative shrink-0 flex gap-2">
                {isRecording ? (
                  <div className="group relative">
                    <Button
                      ...
                      className="h-10 w-10 rounded-full border-destructive/50 text-destructive hover:bg-destructive/10 hover:text-destructive"
                      size="icon"
                    >
                      <X className="h-4 w-4" />
                    </Button>
                ...
                <div className="group relative">
                  <Button
                    ...
                    disabled={isTranscribing || isPending || !selectedProfileId}
                    className="h-10 rounded-full transition-all duration-200 px-3"
                  >
                    {isTranscribing ? (
                      <>
                      ...
                      <>
                        ...
                        <span className="text-xs">Stop capture</span>
                      </>
                    ) : (
                      <>
                        <Mic className="h-4 w-4 mr-2" />
                        <span className="text-xs">Record speech</span>
                      </>
                    )}
                  </Button>
                </div>
                ...
                <div className="group relative">
                  <Button
                    type="submit"
                    disabled={isPending || isVoiceCaptureInProgress || !selectedProfileId}
