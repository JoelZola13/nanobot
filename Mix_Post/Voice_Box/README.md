...
Voicebox includes a lightweight CLI bridge for OpenCode-compatible STT/TTS workflows.

```bash
# List profiles (optional --name filter for discovery)
python3 scripts/opencode_voice_bridge.py profiles

# Transcribe an audio file
python3 scripts/opencode_voice_bridge.py stt /path/to/audio.wav

# Synthesize speech from text using a profile ID
python3 scripts/opencode_voice_bridge.py tts "Hello" --profile-id <profile-id>

# Use the same with the shim for short invocation
bin/voicebox-opencode tts "Hello" --profile-name "Demo Voice"

# Point to a non-default server if needed
VOICEBOX_API_URL=http://127.0.0.1:17493 python3 scripts/opencode_voice_bridge.py --wait tts "Hello" --profile-id YOUR_PROFILE_ID
```

Smoke tests

```bash
# Run with a mock backend (no local server required)
python3 scripts/smoke_opencode_voice_bridge.py --mock

# Or via package script
bun run smoke:opencode-bridge

# Run against a live backend
python3 scripts/smoke_opencode_voice_bridge.py --base-url http://127.0.0.1:17493

# Or via package script / Make target (starts backend if needed)
bun run smoke:opencode-bridge-live
make smoke-opencode-bridge-live
```

The smoke test validates both CLI and MCP flows (including `tools/list` and `tools/call`).

```bash
# Confirm MCP registration from OpenCode's perspective
opencode mcp list --print-logs
```

Enable OpenCode from your user config in one command:
...
Register in OpenCode (local MCP)
...
```json
{
  ...
  "mcp": {
    "voicebox": {
      "type": "local",
      "command": ["/path/to/Voice_Box/bin/voicebox-opencode", "--mcp"],
      "enabled": true
    }
  }
}
...
OpenCode expects `command` as an array, loaded from:
